...
正式结果页必须按 `resultId` 定位本地训练结果，不得继续按 `levelKey` 作为正式结果定位方式。

**Reason**

`levelKey` 表示关卡，不表示某次训练结果。同一关卡可以训练多次，只有 `resultId` 才能唯一绑定某一条 `TrainingResult`。
...
* `/training/[levelKey]/result` 不得继续扩展为正式结果展示页
* 涉及某一条训练结果时必须使用 `resultId`

**Future notes**

后续游客合并、结果回放、分享链接等场景，都应继续沿用 `resultId`。
...
这样可以避免用户不知情写库、避免批量写入错误数据，也能降低 MVP 阶段复杂度。
...
如果后续要做更自动化的合并，必须先新增设计评审与回滚机制。

## ADR-010：本地 syncStatus 只作为 UI 状态，真正幂等由服务端 resultId 保证

**Decision**

本地 `syncStatus` 只用于页面展示和交互提示；真正的保存幂等与归属保护仍然由服务端 `resultId` 机制保证。

**Reason**

localStorage 不可信，也不是最终状态来源；真正权威状态必须由服务端数据库决定。
...
* 不能因为本地 `syncStatus` 丢失就允许覆盖或越权
* 保存失败时不得把本地结果误标为已同步

**Future notes**

后续如果引入更复杂的同步状态，也必须保持“服务端为真相来源”的原则。