...
1. 训练历史和救急历史可以分离展示
2. 展示字段不同的记录类型，不要过早混成一个复杂大页
3. 优先独立入口，后续再评审是否统一信息架构
...
1. 前端提交业务数据
2. 服务端决定归属
3. `userId` 不从 query string、localStorage 或前端参数获取
4. 正式写库身份只来自服务端 session
